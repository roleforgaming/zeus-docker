...
import {
  IDE_LIST, detectInstalledIDEs, isClaudeCodeInstalled, getClaudeCodeVersion,
  getClaudeModelAliases, checkLatestClaudeVersion, updateClaudeCode, getShellEnv
} from './claude-cli.js'
import {
  initClaudeSession, getSession, deleteSession,
  spawnClaudeSession, respondToSession, abortSession, killAllClaudeSessions
...
import {
  listMarkdownFiles, readFileContent, writeFileContent,
  getGitDiff, getGitDiffFile, getGitChangedFiles, readClaudeTranscript
} from './files.js'
import {
  initSubagentWatcher, startSubagentWatch, updateSubagentTargets, stopSubagentWatch
...
if (fs.existsSync(staticDir)) {
  ...
  app.get('/', (_req, res) => res.send('[zeus] Frontend not built yet. Run: npm run build'))
}
...
io.on('connection', (socket) => {
  ...
  socket.on('terminal:create', (workspacePath, cb) => {
    ...
    ptyProcess.onData((data) => socket.emit('terminal:data', { id: termId, data }))
  ...
  socket.on('claude:check-latest', async (cb) => cb(await checkLatestClaudeVersion()))
  ...
  socket.on('ide:open', ({ ideCmd, workspacePath }, cb) => {
    ...
    if (!allowedCmds.has(ideCmd)) return cb({ success: false, error: `Unknown IDE command: ${ideCmd}` })
    try {
      const child = spawnProc(ideCmd, [workspacePath], {
        detached: true, stdio: 'ignore', shell: true, env: getShellEnv()
      })
      child.unref()
      cb({ success: true })
    } catch (e) {
  ...
  socket.on('system:open-external', (url, cb) => {
    ...
    try {
      const child = spawnProc(openCmd, [url], { detached: true, stdio: 'ignore' })
      child.unref()
    } catch { /* best-effort */ }
    cb(true)
  })
  ...
  socket.on('system:reveal-in-finder', (p, cb) => {
    const revealCmd = process.platform === 'darwin' ? 'open'
      : process.platform === 'win32' ? 'explorer' : 'xdg-open'
    const revealArgs = process.platform === 'darwin' ? ['-R', p] : [p]
    try {
      const child = spawnProc(revealCmd, revealArgs, { detached: true, stdio: 'ignore' })
      child.unref()
    } catch { /* best-effort */ }
    cb(true)
  })
  ...
  socket.on('system:get-dir-info', (dirPath, cb) => {
    try {
      ...
      cb({ name: path.basename(dirPath), path: dirPath, hasGit, hasPackageJson, packageName })
    } catch { cb(null) }
  })
  ...
  socket.on('claude-config:write', (config, cb) => cb(writeClaudeConfig(config)))
  socket.on('claude-config:read-project', (wsPath, cb) => cb(readProjectClaudeConfig(wsPath)))
  socket.on('claude-config:write-project', (wsPath, config, cb) => cb(writeProjectClaudeConfig(wsPath, config)))
  ...
  socket.on('plugin:marketplace-list', async (cb) => cb(await listMarketplaces()))
  socket.on('plugin:install', async (name, scope, cb) => cb(await runPluginCmd('install', name, scope)))
  socket.on('plugin:uninstall', async (name, cb) => cb(await runPluginCmd('uninstall', name)))
  socket.on('plugin:enable', async (name, scope, cb) => cb(await runPluginCmd('enable', name, scope)))
  socket.on('plugin:disable', async (name, scope, cb) => cb(await runPluginCmd('disable', name, scope)))
  socket.on('plugin:marketplace-add', async (source, cb) => cb(await runPluginCmd('marketplace add', source)))

  // ── Skills ─────────────────────────────────────────────────────────────────

  socket.on('skills:scan', async (wsPath, cb) => cb(await scanCustomSkills(wsPath)))

  // ── Files / Markdown ───────────────────────────────────────────────────────

  socket.on('files:list-md', (dirPath, cb) => cb(listMarkdownFiles(dirPath)))
  socket.on('files:read', (filePath, cb) => cb(readFileContent(filePath, getStore())))
  socket.on('files:write', (filePath, content, cb) => cb(writeFileContent(filePath, content, getStore())))

  // ── Git ────────────────────────────────────────────────────────────────────

  socket.on('git:diff', async (workspacePath, cb) => cb(await getGitDiff(workspacePath)))
  socket.on('git:diff-file', async (workspacePath, filePath, cb) => cb(await getGitDiffFile(workspacePath, filePath)))
  socket.on('git:changed-files', async (workspacePath, cb) => cb(await getGitChangedFiles(workspacePath)))

  // ── Saved Claude Sessions ──────────────────────────────────────────────────

  socket.on('claude-session:list-saved', (workspacePath, cb) => {
    const saved = getStore().savedSessions ?? []
    cb(saved.filter((s) => s.workspacePath === workspacePath).sort((a, b) => b.lastUsed - a.lastUsed))
  })
  ...
  socket.on('claude-session:read-transcript', (sessionId, workspacePath, cb) => {
    cb(readClaudeTranscript(sessionId, workspacePath))
  })

  // ── Claude Session (PTY-based) ─────────────────────────────────────────────

  socket.on('claude-session:send', (conversationId, prompt, cwd, model, resumeSessionId, cb) => {
    cb(spawnClaudeSession(conversationId, prompt, cwd, model, resumeSessionId))
  })

  socket.on('claude-session:abort', (conversationId, cb) => cb(abortSession(conversationId)))
  ...
  socket.on('claude-session:respond', (conversationId, response, cb) => {
    ...
    if (session?.sessionId && session.cwd) {
      ...
      return cb(spawnClaudeSession(conversationId, response, session.cwd, undefined, session.sessionId))
    }
    cb(false)
  })
  ...
  socket.on('claude-session:watch-subagents', (conversationId, parentSessionId, workspacePath, targets, cb) => {
    cb(startSubagentWatch(conversationId, parentSessionId, workspacePath, targets))
  })
