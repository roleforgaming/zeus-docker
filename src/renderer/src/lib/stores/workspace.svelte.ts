...
class WorkspaceStore {
  ...
  async add(providedPath?: string): Promise<Workspace | null> {
    const wsPath = providedPath ?? prompt("Enter workspace directory path:");
    if (!wsPath || !wsPath.trim()) return null;
    const trimmed = wsPath.trim()
    const result = await window.zeus.workspace.add(trimmed);
    if (!result) {
      uiStore.showToast(
        `Could not add workspace: "${trimmed}" does not exist or is not a directory.`,
        'error'
      );
      return null;
    }
